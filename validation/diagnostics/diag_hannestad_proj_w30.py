...
Fix candidate: projection + T_boltz_start = 30 MeV (sprint-2 auto-scale
...
T_boltz_start = 30 MeV, n_B auto-scaled via validator.
"""
import os
import sys
import time
...
if __name__ == "__main__":
    print("=" * 96)
    print("Stage E.2 sprint-5: Hannestad+2012 WITH projection + T_boltz_start=30 MeV")
    print("qke_v_nunu_active_only = True, T_boltz_start = 30 MeV (auto-scaled n_B).")
    print("=" * 96)

    Neff_3x3, _ = _run("3x3 QKE reference (no sterile, 30 MeV window)", configure_3x3)
    print()
    ...
    print(f"{'Point':30s}  {'sin²2θ':>10s}  {'H+2012':>10s}  "
          f"{'Ours (proj+30)':>15s}  {'Ours (proj+5)':>15s}  {'Ours (orig)':>12s}")
    print("-" * 104)
